...
脚本会先构建 gateway，再以 `HOST=0.0.0.0`、`PORT=3737` 启动当前电脑上的 OpenCodex。Mac 域名只显示 Mac 本机会话，Win 域名只显示 Win 本机会话，不做跨设备会话合并。

## 常驻服务

远程访问时建议把 gateway 做成后台常驻服务。这样手机或另一台电脑打开页面时，会直接连接已经热好的 gateway，避免每次访问都重新启动隐藏 Electron runtime、app-server 和插件同步。
...
默认监听 `0.0.0.0:3737`。如需调整端口或监听地址，安装时传入环境变量，例如：
